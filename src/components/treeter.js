...
const deconsteFromTree = (list, id, idPropName = 'id', childrenPropName = 'children') => {
  ...
  for (var i = 0; i < list.length; i++) {
    if (list[i][idPropName] === id) {
      ...
      const result = deconsteFromTree(list[i][childrenPropName], id, idPropName, childrenPropName)
      if (result) {
        return result
      }
    }
...
const batchDeconsteFromTree = (list, ids, idPropName = 'id', childrenPropName = 'children') => {
  ...
  for (var i = 0; i < list.length; i++) {
    if (ids.findIndex(x => x === list[i][idPropName]) > -1) {
      ...
      batchDeconsteFromTree(list[i][childrenPropName], ids, idPropName, childrenPropName)
    }
  }
}
